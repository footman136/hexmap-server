...
//    ————————————————
//    版权声明：本文为CSDN博主「小竹zz」的原创文章，遵循 CC 4.0 BY-SA 版权协议，转载请附上原文出处链接及本声明。
//    原文链接：https://blog.csdn.net/zhujunxxxxx/article/details/43573879}
namespace IocpServer
{
    ...
    internal sealed class IoServer
    {
        ...
        /// <param name="bufferSize">消息缓冲区的大小（字节数）</param>
        /// <param name="cb">接收消息的回调函数</param>
        internal IoServer(Int32 numConnections, Int32 bufferSize, ReceiveCallBack cb)
        {
...
//                        }
                        // 消息提交外部的回调函数处理
                        receiveCallBack(e, e.Buffer, e.BytesTransferred);
                    }
        ...
        private void ProcessError(SocketAsyncEventArgs e)
        {
            ...
            string outStr = String.Format("套接字错误 {0}, IP {1}, 操作 {2}。", (Int32) e.SocketError, localEp,
                e.LastOperation);
...
#region Close
        ...
        private void CloseClientSocket(Socket s, SocketAsyncEventArgs e)
        {
            ...
            string outStr = String.Format("客户 {0} 断开, 共有 {1} 个连接。", s.RemoteEndPoint.ToString(),
            ...
            if (s.Connected)
            {
                try
                {
                    ...
                    if (ioContext != null)
                    {
                        ...
                        string outStr = String.Format("客户 {0} 连入, 共有 {1} 个连接。", s.RemoteEndPoint.ToString(),
                            this.numConnectedSockets);
                        //mainForm.Invoke(mainForm.setlistboxcallback,outStr);
                        Debug.Log(outStr);
